...
public class CorsConfig {
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        ...
        corsConfiguration.setAllowedHeaders(List.of("*")); //!If you do not add the custom header to the CORS configuration, the browser will not expose it to the JavaScript on the client side, even though it might still be sent by the server and visible in the developer tools. When credentials are involved, wildcard characters like "*" do not work as expected. By default, only a few headers are exposed (Cache-Control, Content-Language, Content-Type, Expires, Last-Modified, Pragma).
